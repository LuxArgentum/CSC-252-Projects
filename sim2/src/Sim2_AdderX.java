...
public class Sim2_AdderX {
    ...
    private AND and1;                       // and gate
    private AND and2;                       // and gate
    private AND and3;                       // and gate
    private AND and4;                       // and gate
    private NOT not1;                        // not gate
    private NOT not2;                       // not gate
    private NOT not3;                       // not gate
    private NOT not4;                       // not gate
    private OR or;                          // or gate
    private OR or2;                         // or gate
    ...
    public void execute() {
        ...
        if (matchingSign && !matchingSignSum) {
            overflow.set(true);
        } else {
            overflow.set(false);
        }
